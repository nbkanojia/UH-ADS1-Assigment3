...
def find_cluster(fert_data_cw,TFRT_data_cw):
   ...
   # print(TFRT_data_cw)
    ...
    df_1990["Fertility rate, total (births per woman)"] = TFRT_data_cw.loc[:,2020].copy() 
  
    df_1990 = df_1990.dropna(axis=0)
   ...
   #      print(f"The silhouette score for {ic: 3d} is {score: 7.4f}")   # allow for minus signs
    ...
    cen_1990 = ct.backscale(cen_1990, df_min_1990, df_max_1990)
    
    xkmeans_1990 = cen_1990[:, 0]
    ykmeans_1990 = cen_1990[:, 1]
    # extract x and y values of data points
    x = df_1990["Fertilizer Consumption"]
    y = df_1990["Fertility rate, total (births per woman)"]
    ...
    cen_2021 = ct.backscale(cen_2021, df_min_2021, df_max_2021)
    
    xkmeans_2021 = cen_2021[:, 0]
    ykmeans_2021 = cen_2021[:, 1]
    # extract x and y values of data points
    x = df_2021["Fertilizer Consumption"]
    y = df_2021["Fertility rate, total (births per woman)"]
...
def fitting(TFRT_data_yw,country):
    ...
    param_b, cov_b = opt.curve_fit(poly3, df_fitting.index,
                                   df_fitting[country])
    ...
    df_fitting['fit'] = poly3(df_fitting.index, *param_b)
    
    # forecasting the fit figures
    forecast_b = poly3(year, *param_b)
    ...
    #sigma = err.error_prop(df_fitting.index, poly3, param_b, cov_b)
    #low = forecast_b - sigma
    #up = forecast_b + sigma
    #plt.fill_between(df_fitting.index, low, up, color="yellow", alpha=0.7)
...
#print(TFRT_data_yw)
#ele_data_cw = ele_data_cw.dropna(axis=0)
#print(ele_data_cw)

#print(death_data_cw[1990])
#find_cluster(TFRT_data_cw,TFRT_data_cw)

print(birth_data_yw)
#find_cluster(death_data_cw.copy(),TFRT_data_cw.copy())
#find_cluster(death_data_cw.copy(),birth_data_cw.copy())



#find_cluster(total_population_data_cw,ele_data_cw)
#ind_cluster(gdp_data_cw,co2_data_cw)
############### Fiting
#print(TFRT_data_yw)
#fitting(TFRT_data_yw,"World")

#fitting(TFRT_data_yw.copy(),"World")

#fitting(death_data_yw.copy(),"World")

# ["High income","Low income"]
fitting(death_data_yw.copy(),"High income")
fitting(death_data_yw.copy(),"Low income")



#find_cluster(total_population_data_cw,TFRT_data_cw)
